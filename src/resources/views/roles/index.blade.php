...
@section('title', 'Roles') {{-- For different pages you can change your title here --}} 
@section('body_content') {{--Put your middle content--}}
<!--*****************************************Page Title*****************************************************-->
<div class="row">
    <div class="col-lg-12">
        <h1 class="page-header"><i class="fa fa-lock"></i>Roles
        <span style=" margin-left:20px"><button onclick="window.location.href='{{ route('roles.create') }}'" type="button" class="btn btn-success fa fa-plus" style="font-size:17px;"> Add New</button></span>
        <span style=" margin-left:10px">
            <form action="{{ route('roles.destroy', 0) }}" method="POST" style="display: inline" onsubmit="return confirm('Are you sure for Bulk Delete?');">
                        ...
                        <tbody id='tbody' >
                            @if (count($roles) > 0)
                                <section class="articles">
                                    @include('tm81_role::roles.index_ajax')
                                </section>
                            @endif
                       </tbody>
                    </table>
